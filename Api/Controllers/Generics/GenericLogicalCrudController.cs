...
public class GenericLogicalCrudController<TEntity, TDto>(IGenericService<TEntity> service) : ControllerBase
{
    [HttpGet]
    ...
    public virtual async Task<IActionResult> DeleteAsync([FromRoute] Guid id,
        CancellationToken cancellationToken = default)
    {
        var result = await service.DeleteLogicalAsync(id, cancellationToken);
        return Ok(result);
    }
}